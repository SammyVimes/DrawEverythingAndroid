...
public class ColorPickerDialog extends Dialog {
	...
	private static class ColorPickerView extends View {
		...
		private OnColorChangedListener mListener;
		private int paddingLeft; float screenCoef;
		ColorPickerView(Context c, OnColorChangedListener l, int color, int defaultColor) {
			super(c);
			//System.out.println(this.getHitRect(outRect));
			
			mContext =c;
				
			 
			 screenCoef = (float) Math.ceil(getResources().getDisplayMetrics().density); 
			 paddingLeft = (int) (15 );
			mListener = l;
		...
		protected void onDraw(Canvas canvas) {
			...
			for (int x=0; x<256; x++)
			{
				...
				canvas.drawRect(x * screenCoef+ paddingLeft, 0, (x+1) * screenCoef+ paddingLeft, 40, mPaint);
			}
			...
			for (int x=0; x<256; x++)
			{
				...
				canvas.drawRect(x * screenCoef + paddingLeft, 50, (x+1) * screenCoef+ paddingLeft, 305, mPaint);
            ...
            if(Utilities.itRuLocale(mContext)) {
                canvas.drawText("Сохранить", 74, 340, mPaint);
            }else{
                canvas.drawText("Save", 74, 340, mPaint);
            }
            ...
            if(Utilities.itRuLocale(mContext)) {
                canvas.drawText("Отменить", 202, 340, mPaint);
            }else{
                canvas.drawText("Undo", 202, 340, mPaint);
            }
	}

		@Override
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			setMeasuredDimension(((int) (276 * screenCoef)), 366);
		}
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			if (x > paddingLeft && x < paddingLeft+266 * screenCoef && y > 0 && y < 40)
			{
				
				// Update the main field colors
				mCurrentHue = (255- (int)((x-paddingLeft)/screenCoef))*360/255;
				updateMainColors();

				// Update the current selected color
				int transX =  mCurrentX-paddingLeft;
			...
			if (x >  paddingLeft && x < paddingLeft+266 * screenCoef && y > 50 && y < 306)
			{
				mCurrentX = (int) x;
				mCurrentY = (int) y;
				int transX = mCurrentX-paddingLeft;
				int transY = mCurrentY-60;
				int index = 256*(transY-1)+(int) (transX / screenCoef);
				if (index > 0 && index < mMainColors.length)
				{
			...
			if (x > paddingLeft && x < 138 && y > 316 && y < 356)
